<?php
    ...
    while(true) {
        ...
        $room_key = FindRoom($date_in, $date_out, $type, $beds);
...
<?php 
    ...
    function FindRoom($date_in, $date_out, $type, $beds) {
            ...
            for ($i = 1; $i <= $GLOBALS['numberOfRooms']; $i++) {
                ...
                if($date_in > $curr_end_date || $date_out < $curr_start_date) { 
                    if($curr_type == $type) {
                        ...
                        $status = "Nessuna stanza di tipo $type è stata trovata per questa data.";
                    }
                } else {
                    $status = "Nessuna stanza $type disponibile per il periodo tra il ". $date_in -> format('d-m-Y') ." e il ". $date_out -> format ('d-m-Y').". Riprova con un'altra data.";
                }  
            }

            if ($beds < 3) {
                // Funzione ricorsiva con numero di letti incrementato di 1
                $status = FindRoom($date_in, $date_out, $type, $beds + 1);
            }
            echo "sono qua";
            return $status;
        }
